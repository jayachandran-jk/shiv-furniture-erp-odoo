...
interface State {
  ...
  updateManufacturingOrder: (id: string, patch: { assigneeId?: string; qty?: number; components?: { productId: string; requiredQty: number }[] }) => Promise<void>;
...
function mapManufacturingOrder(mo: any): ManufacturingOrder {
  ...
  return {
    ...
    shortageTickets: (mo.shortageTickets || []).map((t: any) => ({
      id: t.id,
      moId: t.moId,
      productId: t.productId,
      shortageQty: t.shortageQty,
      poId: t.poId,
      poNumber: t.poNumber,
      status: t.status,
...
export const useERP = create<State>()(
  persist(
    (set, get) => ({
      ...
      updateManufacturingOrder: async (id, patch) => {
        await apiCall(`/api/manufacturing/${id}`, "PUT", patch);
        await get().refreshData();
      },
